...
#
# 1. 首先，对于每个数字d（d=0,1,...,9），我们把所有训练集中属于该数字的图片（每张图片是256维向量）看作一个点云Cd。对于每个点云Cd，可以计算其中心cd（即所有属于该类的向量在每个维度上的均值，得到一个256维的中心向量）。有了这10个中心后，我们可以用“最近中心”法来分类新图片：计算新图片与每个中心的距离，距离最近的中心对应的数字就是分类结果。接着，计算这10个中心两两之间的距离dist_ij（i,j=0,...,9），并据此分析分类器的预期准确率，以及哪些数字对最难区分。
#
