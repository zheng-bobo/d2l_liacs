...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Baseline CNN clock classifier (original lightweight version)."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
...
class ClockDataset:
    """Original dataset pipeline without augmentation or extra blocks."""
...
class CommonSenseCallback(tf.keras.callbacks.Callback):
    """Compute original common-sense metrics after each epoch."""
...
class ClockDataClassificationTrainer:
    """Original lightweight CNN trainer."""
    ...
    def fit(self, epochs):
        callback = CommonSenseCallback(self)
        self.model.fit(
