...
class XORNet:
    ...
    def __init__(
        ...
        self.W1 = (
            torch.normal(mean=0.0, std=weight_scale, size=(2, 2))
            .numpy()
            .astype(self.dtype)
        )
        self.b1 = (
            torch.normal(mean=0.0, std=weight_scale, size=(2,))
            .numpy()
            .astype(self.dtype)
        )
        self.W2 = (
        ...
        self.b2 = (
            torch.normal(mean=0.0, std=weight_scale, size=()).numpy().astype(self.dtype)
        )
    ...
    def forward(self, X: np.ndarray) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        ...
        h = self.act(z1)  # sigmoid/tanh/relu 接受线性输入
